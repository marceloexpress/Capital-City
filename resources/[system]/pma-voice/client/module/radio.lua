...
RegisterCommand('+radiotalk', function()
    ...
    if (LocalPlayer.state.StraightJacket) then return end
        ...
        if radioChannel > 0 then
            ...
            if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) and not disableRadioAnim then
               RequestAnimDict('random@arrests')
				...
				radioOBJ = CreateObject(GetHashKey('prop_cs_hand_radio'),coords.x,coords.y,coords.z,false,false,true)
				SetEntityCollision(radioOBJ,false,false)
				AttachEntityToEntity(radioOBJ,PlayerPedId(),GetPedBoneIndex(PlayerPedId(),60309),0.06,0.05,0.03,-90.0,30.0,0.0,false,false,false,false,2,true)
            end
...
RegisterCommand('-radiotalk', function()
    if (radioChannel > 0 or radioEnabled) and radioPressed then
        ...
        if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			...
			if DoesEntityExist(radioOBJ) then
				if NetworkGetEntityIsNetworked(radioOBJ) then
					TriggerServerEvent("trydeleteobj",ObjToNet(radioOBJ))
				end
				DeleteEntity(radioOBJ)
				radioOBJ = nil
			end
        end
...
if gameVersion == 'fivem' then
    RegisterKeyMapping('+radiotalk', 'Falar na rádio', 'keyboard', GetConvar('voice_defaultRadio', 'CAPITAL'))
end
