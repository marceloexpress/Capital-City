...
camMenu = NativeUI.CreateMenu('Capital Cam', 'Controle a Câmera Cinemática')
...
local CamThread = function()
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent('gb_cam:disableCam')
    end)
...
function GenerateCamMenu()
    ...
    local itemToggleMenu = NativeUI.CreateCheckboxItem(Cam.strings.hideMenuCam, false, Cam.strings.hideMenuCamDesc)
    camMenu:AddItem(itemToggleMenu)

    itemToggleMenu.CheckboxEvent = function(menu, item, checked)
        renderMenu = false
    end
...
function ProcessCamControls()
    ...
    if (isAttached) then
        ...
        if (Vdist(0.0, 0.0, 0.0, offsetCoords.x, offsetCoords.y, offsetCoords.z) > 40.0) then
            TriggerEvent('notify', 'Capital Cam', 'Você se <b>afastou</b> do seu personagem!')
            AttachCamToEntity(cam, entity, offsetCoords.x, offsetCoords.y, offsetCoords.z, true)
        end
